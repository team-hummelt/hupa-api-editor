...
switch ($method) {
    case 'save_license_data':
        ...
        update_option( "hupa_api_editor_client_id", $client_id );
        update_option( "hupa_api_editor_client_secret", $client_secret );

        $responseJson->status = true;
        $responseJson->send_url = apply_filters('get_hupa_api_editor_api_urls', 'authorize_url');
        $responseJson->if_authorize = get_option('hupa_api_editor_product_install_authorize');
